class PickupLinesController < ApplicationController
  before_action :set_pickup_line, only: [:show, :edit, :update, :destroy]

  # GET /pickup_lines
  # GET /pickup_lines.json
  def index
    @pickup_lines = PickupLine.all
  end

  # GET /pickup_lines/1
  # GET /pickup_lines/1.json
  def show
  end
  ...
  # POST /pickup_lines
  # POST /pickup_lines.json
  def create
    @pickup_line = PickupLine.new(pickup_line_params)

    respond_to do |format|
      if @pickup_line.save
        format.html { redirect_to @pickup_line, notice: 'Pickup line was successfully created.' }
  ...
  # PATCH/PUT /pickup_lines/1
  # PATCH/PUT /pickup_lines/1.json
  def update
    respond_to do |format|
      if @pickup_line.update(pickup_line_params)
        format.html { redirect_to @pickup_line, notice: 'Pickup line was successfully updated.' }
  ...
  # DELETE /pickup_lines/1
  # DELETE /pickup_lines/1.json
  def destroy
    @pickup_line.destroy
    respond_to do |format|
  ...
  private
    ...
    def pickup_line_params
      params.require(:pickup_line).permit(:text, :author_id)
    end
end
